...
public final class M05_ProtocolIdentification extends AbstractModule {
  ...
  protected List<Scenario> buildScenarios() {
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M05.1"; }
          public String getTitle() { return "Read ATR and test all protocol rules against it"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            try {
              ...
              for (PcscCardCommunicationProtocol p : PcscCardCommunicationProtocol.values()) {
                ...
                console.info(String.format("  %-25s -> %s", p.name(), match ? "MATCH" : "no match"));
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M05.2"; }
          public String getTitle() { return "isProtocolSupported() for all known protocols"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            for (PcscCardCommunicationProtocol p : PcscCardCommunicationProtocol.values()) {
              ...
              console.info(String.format("  %-25s -> %s", p.name(), ok ? "supported" : "NOT supported"));
              if (ok) supported++;
            }
            ...
            return ScenarioResult.fail(getId(), "Only " + supported + "/" +
                PcscCardCommunicationProtocol.values().length + " protocols reported as supported");
          }
        });

    list.add(
        new Scenario() {
          public String getId() { return "M05.3"; }
          public String getTitle() { return "Custom protocol rule: user-supplied ATR regex"; }
          public String getRequiredEquipment() { return "A card matching the custom rule"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            try {
              ...
              ctx.setPlugin(ctx.getService().registerPlugin(
                  PcscPluginFactoryBuilder.builder()
                      .updateProtocolIdentificationRule(customName, customRegex)
                      .build()));
              console.info("Plugin re-registered with custom rule: " + customName + " = " + customRegex);

              ReaderSpi spi = (ReaderSpi) ctx.getPcscReader(ctx.getPlugin().getReader(reader.getName()));
              ConfigurableReaderSpi cfg = (ConfigurableReaderSpi) spi;
              ObservableReaderSpi obs = (ObservableReaderSpi) spi;
              ctx.getConsole().waitForEnter("Insert a card that should match \"" + customName + "\"");
              boolean present = spi.isCardPresent();
              if (!present) {
                return ScenarioResult.fail(getId(), "isCardPresent() returned false");
              }
              ...
              return ScenarioResult.fail(getId(), "Custom rule did not match; ATR=" + spi.getPowerOnData());
            } catch (Exception e) {
              return ScenarioResult.fail(getId(), e.getMessage());
            }
          }
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M05.4"; }
          public String getTitle() { return "Disable a protocol rule (set to null)"; }
          public String getRequiredEquipment() { return "A card matching ISO_14443_4"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            try {
              ...
              ConfigurableReaderSpi cfg = (ConfigurableReaderSpi) spi;
              ObservableReaderSpi obs = (ObservableReaderSpi) spi;
              ctx.getConsole().waitForEnter("Insert an ISO 14443-4 card to verify rule is disabled");
              boolean present = spi.isCardPresent();
              if (!present) {
                return ScenarioResult.fail(getId(), "isCardPresent() returned false");
              }
              console.info("ATR: " + spi.getPowerOnData());
              boolean match = cfg.isCurrentProtocol(PcscCardCommunicationProtocol.ISO_14443_4.name());
              obs.deselectCard();
