...
public final class M12_ControlCommands extends AbstractModule {
  ...
  protected List<Scenario> buildScenarios() {
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M12.1"; }
          public String getTitle() { return "getIoctlCcidEscapeCommandId() returns platform-correct value"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            if (id == expected) {
              return ScenarioResult.pass(getId(), "IOCTL CCID Escape ID = " + id + " (correct for " + (isWindows ? "Windows" : "Linux/macOS") + ")");
            }
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M12.2"; }
          public String getTitle() { return "transmitControlCommand() GET_FIRMWARE_VERSION (reader-dependent)"; }
          public String getRequiredEquipment() { return "CCID reader supporting firmware version command"; }

          public ScenarioResult run(ValidationContext ctx) {
            CardReader reader = ctx.selectReader();
            if (reader == null) return ScenarioResult.skip(getId(), "No reader selected");
            PcscReader pcsc = ctx.getPcscReader(reader);
            // GET_FIRMWARE_VERSION command varies by reader vendor; use a common ACS command as example
            // ACS ACR122U: 0xFF 0x00 0x48 0x00 0x00
            byte[] cmd;
            String cmdHex = ctx.getConsole().prompt(
            ...
            try {
              ...
              console.warn("Command failed (reader may not support this command): " + e.getMessage());
              return ScenarioResult.skip(getId(), "Reader rejected control command: " + e.getMessage());
            }
          }
        });
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M12.3"; }
          public String getTitle() { return "Two readers operated simultaneously"; }
          public String getRequiredEquipment() { return "Two PC/SC readers, one card each"; }

          public ScenarioResult run(ValidationContext ctx) {
            List<CardReader> readers = new ArrayList<>(ctx.getReaders());
            if (readers.size() < 2) {
              return ScenarioResult.skip(getId(), "Need at least 2 readers (" + readers.size() + " found)");
            }
            ...
            try {
              ...
              if (p1 && p2) {
                return ScenarioResult.pass(getId(), "Both readers operated simultaneously without conflict");
              }
