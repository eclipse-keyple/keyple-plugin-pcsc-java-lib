...
public final class M08_ObservableReader extends AbstractModule {
  ...
  protected List<Scenario> buildScenarios() {
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M08.1"; }
          public String getTitle() { return "CARD_INSERTED event on card presentation"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            CardReaderObserverSpi observer = event -> {
            ...
            obs.setReaderObservationExceptionHandler(
                (info, name, e) ->  {console.warn("Reader obs error: " + e.getMessage());
                  e.printStackTrace();});
            ...
            try {
              ctx.getConsole().waitForEnterOrTimeout(
                  "Present a card on " + reader.getName(), EVENT_TIMEOUT_MS);
              boolean received = latch.await(3, TimeUnit.SECONDS);
              obs.stopCardDetection();
              obs.removeObserver(observer);
              if (received) {
                return ScenarioResult.pass(getId(), "CARD_INSERTED event received on " + reader.getName());
              }
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M08.2"; }
          public String getTitle() { return "CARD_REMOVED event on card withdrawal"; }
          public String getRequiredEquipment() { return "Any card"; }
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M08.3"; }
          public String getTitle() { return "Multiple insert/remove cycles (3 times)"; }
          public String getRequiredEquipment() { return "Any card"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            if (i == cycles && r == cycles) {
              return ScenarioResult.pass(getId(), cycles + " insert+remove cycles detected correctly");
            }
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M08.4"; }
          public String getTitle() { return "stopCardDetection() stops further events"; }
          public String getRequiredEquipment() { return "Any card"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            ctx.getConsole().waitForEnterOrTimeout(
                "Insert/remove card again (no event should fire)", 5_000);
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M08.5"; }
          public String getTitle() { return "Custom monitoring cycle duration (2000 ms)"; }
          public String getRequiredEquipment() { return "Any card"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            try {
              ...
              ctx.getConsole().waitForEnterOrTimeout(
                  "Insert a card on " + reader.getName(), EVENT_TIMEOUT_MS);
              ...
              if (received) {
                ...
                return ScenarioResult.pass(getId(),
                    "CARD_INSERTED with 2s cycle; delay=" + detectionDelay[0] + "ms");
              }
