...
public final class M09_PresenceMonitoring extends AbstractModule {
  ...
  protected List<Scenario> buildScenarios() {
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M09.1"; }
          public String getTitle() { return "monitorCardPresenceDuringProcessing() returns normally when card stays"; }
          public String getRequiredEquipment() { return "Any card (keep it in the reader)"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            ctx.getConsole().waitForEnter("Insert a card into " + reader.getName() + " and keep it there");
            try {
              ...
              if (monitorException[0] instanceof org.eclipse.keyple.core.plugin.TaskCanceledException) {
                return ScenarioResult.pass(getId(), "Monitor stopped cleanly via TaskCanceledException");
              }
              if (monitorException[0] != null) {
                return ScenarioResult.fail(getId(), "Unexpected exception: " + monitorException[0].getMessage());
              }
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M09.2"; }
          public String getTitle() { return "Card removal detected during monitorCardPresenceDuringProcessing()"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            try {
              ...
              if (monitorException[0] != null) {
                ...
                return ScenarioResult.pass(getId(), "Card removal detected: " + monitorException[0].getClass().getSimpleName());
              }
              return ScenarioResult.pass(getId(), "monitorCardPresenceDuringProcessing() returned normally on card removal");
            } catch (Exception e) {
              return ScenarioResult.fail(getId(), e.getMessage());
            }
          }
    ...
    list.add(
        new Scenario() {
          public String getId() { return "M09.3"; }
          public String getTitle() { return "stopCardPresenceMonitoringDuringProcessing() cancels the wait"; }
          public String getRequiredEquipment() { return "Any card (keep it in the reader)"; }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            ObservableReaderSpi obs = (ObservableReaderSpi) spi;
            CardPresenceMonitorBlockingSpi monitor = (CardPresenceMonitorBlockingSpi) spi;
            ctx.getConsole().waitForEnter("Insert a card into " + reader.getName() + " and keep it");
            try {
              boolean present = spi.isCardPresent();
              if (!present) return ScenarioResult.fail(getId(), "isCardPresent() returned false");
              ...
              if (done) {
                console.info("Cancellation elapsed: " + elapsed + " ms; TaskCanceledException=" + cancelled[0]);
