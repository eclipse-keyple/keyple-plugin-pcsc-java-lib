...
public interface PcscReader extends KeypleReaderExtension {
  ...
  /**
   * Changes the action to be taken when the internal {@code disconnectCard()} method is called
   ...
   * SCARD_UNPOWER_CARD) before card-removal detection, so the internal {@code disconnectCard()}
   * method is typically a no-op and this setting has no effect.
   *
