  ...
  protected List<Scenario> buildScenarios() {
    ...
    list.add(
        new Scenario() {
          ...
          public String getTitle() {
            return "READER_CONNECTED event on USB reader plug-in";
          }

          public String getRequiredEquipment() {
            return "A USB PC/SC reader to plug in";
          }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            try {
              ctx.getConsole()
                  .waitForEnterOrTimeout("Plug in a USB PC/SC reader and press ENTER", 30_000);
              ...
              return ScenarioResult.fail(
                  getId(), "No READER_CONNECTED event received within timeout");
            } catch (Exception e) {
              obs.removeObserver(observer);
    ...
    list.add(
        new Scenario() {
          ...
          public String getTitle() {
            return "READER_DISCONNECTED event on USB reader unplug";
          }

          public String getRequiredEquipment() {
            return "A USB PC/SC reader to unplug";
          }
          ...
          public ScenarioResult run(ValidationContext ctx) {
            ...
            try {
              ctx.getConsole()
                  .waitForEnterOrTimeout("Unplug a USB PC/SC reader and press ENTER", 30_000);
              boolean received = latch.await(5, TimeUnit.SECONDS);
              obs.removeObserver(observer);
              if (received) {
                console.info("READER_DISCONNECTED: " + disconnectedReader[0]);
                return ScenarioResult.pass(
                    getId(), "READER_DISCONNECTED -> " + disconnectedReader[0]);
              }
    ...
    list.add(
        new Scenario() {
          ...
          public String getTitle() {
            return "Plugin monitoring cycle duration (1000 ms)";
          }
