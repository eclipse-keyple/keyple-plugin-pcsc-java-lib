...
import org.eclipse.keyple.plugin.pcsc.it.util.AtrDecoder;
  ...
  protected List<Scenario> buildScenarios() {
    ...
    for (PcscCardCommunicationProtocol proto : PcscCardCommunicationProtocol.values()) {
      ...
      list.add(
          new Scenario() {
            public String getId() {
              return "M04." + (p.ordinal() + 1);
            }

            public String getTitle() {
              return "Detect " + p.name() + " card";
            }
            ...
            public ScenarioResult run(ValidationContext ctx) {
              ...
              ctx.getConsole()
                  .waitForEnter("Insert a " + p.name() + " card into " + reader.getName());
              try {
                ...
                return ScenarioResult.fail(getId(), "Protocol not matched; ATR: " + atr);
              } catch (Exception e) {
    ...
    list.add(
        new Scenario() {
          ...
          public String getTitle() {
            return "Unknown card type: read ATR and display decoded info";
          }
